...
double pippard_reduced_field_penetration(const double *x, const double *par) {
  return std::abs(triumf::superconductivity::pippard::reduced_field_penetration<double>(
      *x, par[0], par[1], par[2], par[3], par[4], par[5], par[6]));
}

double bcs_reduced_field_penetration(const double *x, const double *par) {
  return std::abs(triumf::superconductivity::bcs::reduced_field_penetration<double>(
      *x, par[0], par[1], par[2], par[3], par[4], par[5], par[6]));
}

void plot_field_penetration() {
  ...
  TF1 *f_pippard =
      new TF1("f_pippard", pippard_reduced_field_penetration, z_min, z_max, 7);
  f_pippard->SetTitle("");
  ...
  f_pippard->GetHistogram()->GetXaxis()->SetTitle("z (nm)");
  f_pippard->GetHistogram()->GetYaxis()->SetTitle("| B(z) / B(0) |");

  f_pippard->Draw();

  TF1 *f_bcs = new TF1("f_bcs", bcs_reduced_field_penetration, z_min, z_max, 7);
  f_bcs->SetTitle("");
  ...
  f_bcs->GetHistogram()->GetXaxis()->SetTitle("z (nm)");
  f_bcs->GetHistogram()->GetYaxis()->SetTitle("| B(z) / B(0) |");
  ...
  f_pippard->GetYaxis()->SetRangeUser(1e-4, 2e0);

  canvas->Print("field_penetration.pdf", "EmbedFonts");
}
