...
void dumb_minimum_search() {
  ...
  */
  ...
  auto n_tries = 3;
  /*
  auto p0 = triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(0) -
  fitter.Config().ParSettings(i).Error(0),
  fitter.Config().ParSettings(i).Value(0) +
  fitter.Config().ParSettings(i).Error(0), n_tries); auto p1 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(1) -
  fitter.Config().ParSettings(i).Error(1),
  fitter.Config().ParSettings(i).Value(1) +
  fitter.Config().ParSettings(i).Error(1), n_tries); auto p2 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(2) -
  fitter.Config().ParSettings(i).Error(2),
  fitter.Config().ParSettings(i).Value(2) +
  fitter.Config().ParSettings(i).Error(2), n_tries); auto p3 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(3) -
  fitter.Config().ParSettings(i).Error(3),
  fitter.Config().ParSettings(i).Value(3) +
  fitter.Config().ParSettings(i).Error(3), n_tries); auto p4 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(4) -
  fitter.Config().ParSettings(i).Error(4),
  fitter.Config().ParSettings(i).Value(4) +
  fitter.Config().ParSettings(i).Error(4), n_tries); auto p5 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(5) -
  fitter.Config().ParSettings(i).Error(5),
  fitter.Config().ParSettings(i).Value(5) +
  fitter.Config().ParSettings(i).Error(5), n_tries); auto p6 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(6) -
  fitter.Config().ParSettings(i).Error(6),
  fitter.Config().ParSettings(i).Value(6) +
  fitter.Config().ParSettings(i).Error(6), n_tries); auto p7 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(7) -
  fitter.Config().ParSettings(i).Error(7),
  fitter.Config().ParSettings(i).Value(7) +
  fitter.Config().ParSettings(i).Error(7), n_tries); auto p8 =
  triumf::numpy::linspace(fitter.Config().ParSettings(i).Value(8) -
  fitter.Config().ParSettings(i).Error(8),
  fitter.Config().ParSettings(i).Value(8) +
  fitter.Config().ParSettings(i).Error(8), n_tries);
  */

  std::vector<std::vector<double>> mpar;
  for (int i = 0; i < f_3d->GetNpar(); ++i) {
    std::vector<double> vpar = triumf::numpy::linspace<double>(
        fitter.Config().ParSettings(i).Value() -
            fitter.Config().ParSettings(i).Error(),
        fitter.Config().ParSettings(i).Value() +
            fitter.Config().ParSettings(i).Error(i),
        n_tries);
  }

  // perform a very primitive search for a minimum
  for (int i = 0; i < f_3d->GetNpar(); ++i) {
