...
double pippard_reduced_kernel(const double *x, const double *par) {
  return triumf::superconductivity::pippard::reduced_kernel<double>(
      *x, par[0], par[1], par[2], par[3], par[4], par[5], par[6]);
}

double bcs_reduced_kernel(const double *x, const double *par) {
  return triumf::superconductivity::bcs::reduced_kernel<double>(
      *x, par[0], par[1], par[2], par[3], par[4], par[5], par[6]);
}

void plot_reduced_kernel() {
  ...
  TF1 *f_pippard =
      new TF1("f_pippard", pippard_reduced_kernel, q_min, q_max, 7);
  f_pippard->SetTitle("");
  ...
  f_pippard->GetHistogram()->GetYaxis()->SetTitle("K(q) / K(0)");

  f_pippard->Draw();

  TF1 *f_bcs = new TF1("f_bcs", bcs_reduced_kernel, q_min, q_max, 7);
  f_bcs->SetTitle("");
  ...
  f_bcs->GetHistogram()->GetYaxis()->SetTitle("K(q) / K(0)");
  ...
  canvas->Print("reduced_kernel.pdf", "EmbedFonts");
}
